...
def compute_FRweight(FR, branches, Lepton, cut):
  ...
  #ConeCorrectedPt = Lep_pt*(1+branches['data'][f'{Lepton}_pfRelIso03_all'][cut])
...
def apply_FR_2Ele(FR, branches, channel, hist_name, PlotRegion):
  ...
  for XXX in application_region:
    ...
    if XXX == 'PFF':
      weights2 = compute_FRweight(FR, branches, 'Electron1', cut)
      weights3 = compute_FRweight(FR, branches, 'Electron2', cut)

...
'''
#LeptonFlavorr = 'Electron'
baseline_perid = '2018'
period_to_compare = '2016_HIPM'
#regionFR = 'ttbarRegionFR' #DYRegionFR

LeptonFlavorrs = ['Electron', 'Muon', 'Tau']
regionFRs = ['ttbarRegionFR', 'DYRegionFR']
for LeptonFlavorr in LeptonFlavorrs:
    for regionFR in regionFRs:
        # Parameters -----------------------------------------------------------------------------------------------------
        # First FR to compare
        path_to_json_FR1 = f'B_FakeRate/results/FinalProd/{period_to_compare}/FakeFactors{LeptonFlavorr}/{regionFR}/'
        json_file_FR1 =    f'P_fake_TL_{LeptonFlavorr}'
        # Second FR to compare
        path_to_json_FR2 = f'B_FakeRate/results/FinalProd/{baseline_perid}/FakeFactors{LeptonFlavorr}/{regionFR}/'
        json_file_FR2 =    f'P_fake_TL_{LeptonFlavorr}'
        # Name of the out figure
        output_figures =   f'B_FakeRate/figures/FinalProd/{period_to_compare}/FakeFactors{LeptonFlavorr}/{regionFR}/diff_FR_{period_to_compare}_{baseline_perid}.pdf'
        # Label for figure
        if LeptonFlavorr in ['Electron','Muon']:
            x_label = r'$p_T^{corr}$ [GeV]' #r'$p_T^{corr}$ [GeV]'
        else:
            x_label = r'$p_T$ [GeV]'
        if regionFR == 'ttbarRegionFR':
            y_label = fr'$\Delta FF (FF_{{TT}}^{{{baseline_perid}}} - FF_{{TT}}^{{{period_to_compare}}})$'
        elif regionFR == 'DYRegionFR':
            y_label = fr'$\Delta FF (FF_{{DY}}^{{{baseline_perid}}} - FF_{{DY}}^{{{period_to_compare}}})$'
        # ----------------------------------------------------------------------------------------------------------------
